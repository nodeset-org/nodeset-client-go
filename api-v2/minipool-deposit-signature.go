...
func (c *NodeSetClient) MinipoolDepositSignature(ctx context.Context, address common.Address, saltString string) (MinipoolDepositSignatureData, error) {
	...
	if err != nil {
		return MinipoolDepositSignatureData{}, fmt.Errorf("error marshalling login request: %w", err)
	}
	...
	if err != nil {
		return MinipoolDepositSignatureData{}, fmt.Errorf("error requesting whitelist signature: %w", err)
	}
	...
	return MinipoolDepositSignatureData{}, fmt.Errorf("nodeset server responded to minpool deposit signature request with code %d: [%s]", code, response.Message)
}
